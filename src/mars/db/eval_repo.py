...
class EvalRepository:
    ...
    def get_latest_run(self) -> int:
        all_runs = self.runs.all()
        if not all_runs:
            return 0
        return max(entry["run"] for entry in all_runs) + 1

    def save_eval_doc(self, eval_doc: EvalDoc) -> None:
    ...
    def save_scores(self, scores: list[ScoreEntry]) -> None:
        for score in scores:
    ...
    def set_score(self, score: ScoreEntry) -> None:
        q = Query()
        match = (
            (q.run == score.run)
    ...
    def delete_runs_from(self, run_threshold: int) -> None:
        run = Query()
